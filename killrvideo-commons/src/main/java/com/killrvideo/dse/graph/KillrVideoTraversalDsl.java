 ...
 */
@GremlinDsl(traversalSource = "com.killrvideo.dse.graph.KillrVideoTraversalSourceDsl")
public interface KillrVideoTraversalDsl<S, E> extends GraphTraversal.Admin<S, E>, KillrVideoTraversalConstants {
    ...
    /**
     * Traverses from a "user" to a "video" over the "rated" edge, filtering those edges as specified. 
	...
	public default GraphTraversal<S, Vertex> rated(UUID userId, Integer rating) {
         ...
         */
        return ((KillrVideoTraversal) as("^video")).coalesce(__
                .user(userId)
                .addE(EDGE_RATED)
                .property(KEY_RATING, rating)
                .to("^video")
                .inV());
    }
	...
	public default GraphTraversal<S, Vertex> uploaded(UUID userId) {
         ...
         * within the DSL itself.
         */
        return ((KillrVideoTraversal) as("^video")).coalesce(
                 __.uploaders().has(KEY_USER_ID, userId),
                 __.V().user(userId).addE(EDGE_UPLOADED).to("^video").inV());
    }
